1. `spf.cpp`: Smallest Prime Factor is a technique that allows factoring a number N in O(log(N)) time, with preprocessing with prime sieve taking O(log(log(N))) time.
